...
export default function KeybindingModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div 
        className="w-[500px] bg-[#1e1e1e] border border-aether-border rounded-xl shadow-2xl overflow-hidden flex flex-col"
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-[#252526]">
          ...
          <button onClick={onClose} className="text-aether-muted flex items-center justify-center hover:text-white transition">
        ...
        <div className="p-4 grid grid-cols-2 gap-x-6 gap-y-3">
          {shortcuts.map(s => (
            <div key={s.action} className="flex justify-between items-center text-xs">
              <span className="text-aether-muted">{s.action}</span>
              <span className="bg-[#2d2d2d] text-aether-blue px-2 py-0.5 rounded font-mono border border-[#3d3d3d] shadow-sm">{s.key}</span>
            </div>
          ))}
        </div>
      </div>
