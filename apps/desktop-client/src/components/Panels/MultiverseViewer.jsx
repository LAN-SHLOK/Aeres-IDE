export default function MultiverseViewer() {
  return (
    <div className="flex flex-col h-full bg-aether-bg">
      <div className="px-4 py-3 border-b border-aether-border bg-aether-surface flex justify-between items-center">
        <span className="text-xs font-bold text-white uppercase tracking-widest flex items-center gap-2">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-aether-violet"><path d="m22 8-6 4 6 4V8Z"/><circle cx="8" cy="12" r="6"/></svg>
          Multiverse AI Generations
        </span>
      </div>
      <div className="flex-1 flex overflow-hidden">
        {[1,2,3].map(i => (
          <div key={i} className="flex-1 border-r border-aether-border last:border-0 flex flex-col hover:bg-white/5 transition-colors cursor-pointer group">
            ...
            <div className="p-3 text-center opacity-0 group-hover:opacity-100 transition-opacity mt-auto">
              <button className="px-4 py-1.5 bg-aether-surface border border-aether-border rounded text-[10px] font-bold text-white hover:bg-aether-violet hover:border-aether-violet transition-colors">
                Apply Variant
              </button>
            </div>
          </div>
