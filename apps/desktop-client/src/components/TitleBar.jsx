    ...
    >
      ...
      <div className="w-20 flex items-center gap-1.5 pl-4" style={{ WebkitAppRegion: 'no-drag' }}>
        {isCutie && !window.electron?.isMac ? (
          <>
            <button onClick={handleClose} className="w-3 h-3 rounded-full bg-[#ff5f56] border border-black shadow-[1px_1px_0px_#000] focus:outline-none transition-transform hover:scale-110 active:translate-y-[1px]" title="Close" />
            <button onClick={handleMinimize} className="w-3 h-3 rounded-full bg-[#ffbd2e] border border-black shadow-[1px_1px_0px_#000] focus:outline-none transition-transform hover:scale-110 active:translate-y-[1px]" title="Minimize" />
            <button onClick={handleMaximize} className="w-3 h-3 rounded-full bg-[#27c93f] border border-black shadow-[1px_1px_0px_#000] focus:outline-none transition-transform hover:scale-110 active:translate-y-[1px]" title="Maximize" />
          </>
        ) : (
          <div className="w-20" />
        )}
      ...
      <div className="flex-1 flex justify-center px-4">
        <div 
          className={`w-full max-w-[450px] px-3 py-1 flex items-center gap-2 group transition-all cursor-pointer ${
            isCutie 
              ? 'bg-[#fffdf9] border-2 border-black rounded-full shadow-[2px_2px_0px_#000] hover:translate-x-[0.5px] hover:translate-y-[0.5px] hover:shadow-[1.5px_1.5px_0px_#000]' 
        ...
        {!window.electron?.isMac && (
          <>
            <button 
              ...
              className={`w-10 h-10 flex items-center justify-center transition-colors ${isCutie ? 'hover:bg-black/5 text-black' : 'hover:bg-white/5 text-slate-400 hover:text-white'}`}
            >
            ...
            <button 
              onClick={handleClose}
              className={`w-10 h-10 flex items-center justify-center transition-colors ${isCutie ? 'hover:bg-red-500 hover:text-white text-black' : 'hover:bg-red-500/80 text-slate-400 hover:text-white'}`}
            >
