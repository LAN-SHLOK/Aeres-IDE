export default function StagingArea({ file, onUnstage, onDiff }) {
  ...
  return (
    <div 
      className="flex items-center justify-between p-1.5 px-3 hover:bg-aether-surface/40 transition-colors cursor-pointer group rounded"
      onClick={() => onDiff && onDiff(file.path, true)}
    >
      <div className="flex flex-col min-w-0 flex-1">
        <span className="text-xs text-green-400 font-medium truncate">{name}</span>
        <span className="text-[9px] text-aether-muted truncate opacity-80">{file.path}</span>
      </div>
      <button 
        onClick={(e) => { e.stopPropagation(); onUnstage && onUnstage(file.path); }}
        className="text-aether-muted hover:text-aether-red p-1 rounded opacity-0 group-hover:opacity-100 transition-opacity"
        title="Unstage file"
      >
